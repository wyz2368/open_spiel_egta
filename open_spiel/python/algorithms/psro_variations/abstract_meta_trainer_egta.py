...
def self_play_strategy(solver):
    """
    Finding best response to opponents' newest strategies.
    :param solver: GenPSROSolver instance.
    :return: distribution with 1 on the newest strategy.
    """
    policies = solver.get_policies
    if not isinstance(policies[0], list):
        policies = [policies]
    policy_lengths = [len(pol) for pol in policies]
    meta_strategy = [np.zeros(pol_len) for pol_len in policy_lengths]
    for i in range(len(meta_strategy)):
        meta_strategy[i][-1] = 1
    return meta_strategy

def prioritized_FP_strategy(solver):
    pass
...
META_METHODS_VARYING = {
    "nash": general_nash_strategy,
    "uniform": uniform_strategy,
    "selfplay": self_play_strategy,
}

META_METHODS_LIST = [general_nash_strategy, uniform_strategy, self_play_strategy]
...
class AbstractMetaTrainer(object):
  ...
  def __init__(self,
    ...
    self._meta_method_varying = meta_method_varying

  #TODO: how to compute the meta_method_dist?
  def update_meta_method(self, meta_method_dist):
      self._meta_strategy_method = np.random.choice(META_METHODS_LIST, meta_method_dist)
